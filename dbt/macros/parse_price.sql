{#
  ...
  Both implementations exist because the CDC job (Python/Spark) and the dbt models are
  independent consumers of the same Bronze data — see cdc_job.py's module docstring for
  why that duplication is normal in a medallion architecture, not an oversight. Kept in
  sync deliberately: same currency-format table (docs/FINDINGS.md), same "always store
  money as amount*100" convention.
  ...
  Bug found and fixed (2026-08-21): dbt-duckdb's seed loader imports an empty
  ...
  Found because real IDR poll data showed up with `lowest_sell` entirely NULL
