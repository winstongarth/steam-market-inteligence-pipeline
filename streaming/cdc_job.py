"""Streaming CDC and Silver normalization.
...
Tier A polls every 5 minutes — the fastest cadence in the system. A watermark
...
cover CS2/Dota 2/TF2/Rust/PUBG's item taxonomies. Only CS2 (`appid=730`) has ever had real
data fetched — the other four app_ids were never sampled. This schema only relies on
...
## Spread / depth — scoped out of this file's CDC diffing, on purpose
...
blur that scope. They're ready to compute in the Gold layer (`mart_item_daily`)
...
def detect_changes(key, pdf_iter, state: GroupState):  # type: ignore[no-untyped-def]
    ...
    server-inferred and NOT currently controllable (a known finding).
