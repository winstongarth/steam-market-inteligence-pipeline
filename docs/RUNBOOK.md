...
# docs/DECISIONS.md (Phase 2) for the spark-submit invocation and why.
...
and `dim_item` depends on the snapshot. Running the plain `dbt run && dbt snapshot` shortcut
shown in "Cold start" above works too as long as staging models build before the snapshot
runs, which the default DAG order (`dbt seed && dbt run && dbt snapshot && dbt test`)
already guarantees for a from-scratch build.
...
|---|---|---|
| `STEAM_RPS` | `0.5` | Global rate limit, measured in `docs/PHASE0_FINDINGS.md` — do not raise without re-measuring |
