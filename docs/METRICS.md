...
- **The ingestion pipeline's 24h zero-429 soak test has never run.** Everything downstream
  that would benefit from continuous steady-cadence data (CDC compression ratio,
  anomaly-detector hit rates, Bronze partition growth) is measured against a short,
  disconnected ad-hoc run instead, and flagged as such wherever it appears below.
...
- **Snowflake has never been opened.** Deliberately deferred (see `docs/DECISIONS.md`'s
  DuckDB-only warehouse decision) pending the two gaps above — DuckDB carries the whole
  warehouse layer instead, which is also why the query-tuning work used `EXPLAIN ANALYZE`
  instead of Snowflake query profiles, and why "add a clustering key" isn't in this
  project's fix list (DuckDB has no clustering-key equivalent).

## Ingestion & rate limiting

| Metric | Value | Method |
|---|---|---|
| Search/render page size (unauthenticated) | 10 items/request (not the assumed ~100) | Requested `count=10/100/150` against `search/render`, all returned `pagesize: 10`. See `docs/FINDINGS.md`. |
| CS2 (`appid=730`) total catalog size | 35,349 items | `total_count` field, `search/render` response, 2026-08-20. |
| Rate limit — requests before first 429 | 21 requests | Ramp test, 2.0s→0.5s step-down, `priceoverview` endpoint. See `docs/evidence/rate-limit-probe.json`. |
...
| End-to-end smoke test | search_render → priceoverview → orderbook (Tier A path) → Kafka → Bronze Parquet, all real network calls | `scripts/smoke_test.py`, 2026-08-20 |
| Watchlist requests, first batch | 20/20 succeeded, then 3 consecutive 429s | `scripts/fx_poll.py` at `STEAM_RPS=0.5`, halted by the real circuit breaker after item 5. `docs/DECISIONS.md`. |
...
session:**
- 24h continuous polling with zero 429s and no gaps
...
`docs/evidence/rate-limit-probe.json`-style logs for 429 counts and Bronze partition row
counts via DuckDB.
...
polling — needs the ingestion pipeline's 24h soak test data, not this ad-hoc backlog.

## Warehouse (dbt)

DuckDB only for now — Snowflake deliberately deferred, see `docs/DECISIONS.md`.
...
| `fct_price_observation` row count | 16,110 | Matches the Streaming & CDC section's independently-measured "item-level raw observations" count exactly — cross-validated across two independently-built pipelines (Spark streaming vs. dbt/DuckDB batch) on the same source data |
| `fct_price_change` row count | 19,520 | Matches the Streaming & CDC section's corrected CDC event count exactly |
...
| Bug found and fixed: OHLC currency-mixing | `open=3925, high=621400` for one item/day, pre-fix | Third instance of the money-domain bug class (see the Streaming & CDC section above and `docs/DECISIONS.md`) — found by spot-checking a real `mart_item_daily` row before trusting the model, not assumed correct because tests passed |
...
**Not yet done (deliberately):** Snowflake external stage, streams, tasks, 24h unattended
run. Pending the ingestion pipeline's soak test and the orchestration layer's stability.

## Orchestration & quality gates

| Metric | Value | Method |
|---|---|---|
| DAG tasks, full run | 11/11 succeeded | `airflow dags test steam_market_batch 2026-08-21` inside `steam-airflow-webserver`, 2026-08-21. Evidence: `docs/evidence/dag-run-success.txt` |
| Blocking gates that actually blocked on injected bad data | 2/2 tested, both blocked | GX Bronze suite (3 injected failure modes) + a real dbt uniqueness test (injected duplicate row, dbt exit code 1). Evidence: `docs/evidence/quality-gate-failure-great-expectations.txt`, `docs/evidence/quality-gate-failure-dbt.txt` |
| dbt tests, full suite | 21 total: 18 pass, 3 warn (by design), 0 error | `dbt test` against real warehouse, 2026-08-21 |
...
| Real bugs found/fixed building this | 6 (1 data-modeling bug, 5 Airflow/dbt-CLI mechanics) | Full list in `docs/DECISIONS.md`'s orchestration entry |
...
via `airflow dags test` against real data instead. Same gap as the ingestion pipeline's
still-pending 24h soak test.
...
doesn't span enough calendar time. Re-run once the ingestion pipeline's 24h+ soak test
produces real continuous history.
...
| dbt tests, full suite | 26 total: 23 pass, 3 warn (pre-existing, by design), 0 error | `dbt test` after the cross-currency rebuild, 2026-08-21. |
...
gaps in the n=5 partial sample (documented in `docs/DECISIONS.md`) were rounding noise on
cheap items, not a real signal. This is a negative result, stated plainly rather than
reframed as a positive finding.

## Query performance tuning

**Not measured: Snowflake query profiles / clustering-key tuning.** Snowflake was never
reached (deferred per the warehouse decision in `docs/DECISIONS.md`, pending the
ingestion pipeline's soak test and the orchestration layer's stability, both still open).
DuckDB `EXPLAIN ANALYZE` used instead — see method column.
Full run: `scripts/query_tuning.py`, output in `/tmp/query_tuning.log`.
